...
已内置两种广告位，配置后即可展示：**激励视频**（点“保存”先看广告解锁）+ **底部 Banner**。

**开通条件**：小程序**已发布上线** + **历史累计独立访客 UV ≥ 1000** + 无违规记录。

**配置步骤**：
1. 小程序后台 →「流量主」→ 开通。
2. 「流量主 → 广告位」分别新建一个**激励视频**广告位和一个 **Banner** 广告位，
   各得到一个形如 `adunit-xxxxxxxx` 的广告位 ID。
3. 打开 `miniprogram/config.js`，把两个占位 ID 换成你的真实 ID：
   ```js
   rewardedVideoAdUnitId: 'adunit-你的激励视频ID',
   bannerAdUnitId: 'adunit-你的BannerID',
   ```

> 只要保持占位值，广告会**自动关闭**，不影响功能；激励视频加载失败时也会**直接放行**保存，
> 不会因为广告基建问题卡住用户。
